					...
					.pipe(tap(function(file) {
						...
						file.contents = Buffer.from(
							...
							file.contents
								...
								.trim()
								// исправляем в стилях url(...)
								.replace(/(url\(['"]?(?!['"]?data:|\/))(.+?)(['"]?\))/gim, '$1'+urlPrefix+'$2$3')
								// Удаляем возможные sourceMappingURL уже включенные в css-bundle
								.replace(/\n{0,2}\/\*#\s*sourceMappingURL(.*?)\*\//, '')
								// Заменяем конструкции вида ../css/../images на ../images.
								// Такие конструкции появляются после добавления urlPrefix поскольку scss и css могут лежать на разной глубине
								.replace(/(url\(['"]?(?:.*?))(\.\.\/[^.]+\/\.\.\/)((?:.*?)['"]?\))/gim, '$1../$3')
								// Заменяем конструкции вида /./ на /
								.replace(/(url\(['"]?(?:.*?))(\/\.\/)((?:.*?)['"]?\))/gim, '$1/$3')
								.trim()
							,'utf-8'
						);
